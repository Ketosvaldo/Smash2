...
public class ControlMarioBaseChafa : MonoBehaviour
{
    ...
    {
        Debug.Log(jabTiempo);
        //Debug.Log(isRecovery);
        //Movimiento
        if (Input.GetKey(KeyCode.D) && puedoMoverme)
        {
        ...
        else if (Input.GetKey(KeyCode.A) && puedoMoverme)
        {
            if (this.transform.localScale.x > 0 && isGround)
                this.transform.localScale = new Vector2(-this.transform.localScale.x, this.transform.localScale.y);
            if (correr && isGround && !isAttacking)
        ...
        else
        {
            ...
            correr = false;
            if ((Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D)) && isGround && !isAttacking)
                ChangeAnimationState(Player_caminar);
            else if (isGround && !isAttacking)
                ChangeAnimationState(Player_idle);
            else if (!isGround && !isAttacking & !dobleSalto && isRecovery)
